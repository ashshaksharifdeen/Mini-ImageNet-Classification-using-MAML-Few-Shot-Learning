...
from load_data import TaskGenerator
...
def maml_train(model, batch_generator):
    ...
    def _maml_train_step(batch_set):
        ...
        with tf.GradientTape() as outer_tape:
            ...
            #it will loop through each batch
            for idx, task in enumerate(batch_set):
                # Set up copied model
                copied_model = model
                # Slice task(each batch) to support set and query set
                support_x, support_y, query_x, query_y = task
                if visual:
                ...
                # Update fast weights several times
                #Number of inner gradient updates for each task
        ...
        #here it will update the gradient for each epoch/total_batches  
    ...
    start = time.time()
    #here epoch =total_batches
    for step in range(total_batches+1):
        # Get a batch data for training
...
if __name__ == '__main__':
    ...
    argparse.add_argument('--total_batches', type=int, help='Total update steps for each epoch', default=40)
    # Log options
    ...
    argparse.add_argument('--ckpt_dir', type=str, help='Path to the checkpoint directory', default='weights/train/')
